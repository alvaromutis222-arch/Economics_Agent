...
def build_context_profile(df: pd.DataFrame):
    num = df.select_dtypes(include=np.number)
    if num.empty: return "Sin métricas numéricas."
...
def system_prompt(df: pd.DataFrame, persona: str) -> str:
    ...
    Usa el perfil del dataset si preguntan por datos; de lo contrario, responde con teoría y razonamiento transparente.
    ...
    - Ofrece marcos (IS–LM, AS–AD, Taylor, Mundell-Fleming) cuando ayuden.
...
with st.sidebar:
    ...
    key = ensure_groq()
    key = st.text_input("GROQ_API_KEY", value=key, type="password", placeholder="sk_...", help="Define en Secrets o variable de entorno.")
...
with tab_chat:
    ...
    for msg in st.session_state["history"]:
        ...
        st.markdown(f"""<div class="chat"><div class="bubble {klass}">
        <div class="role">{role}</div><div>{msg['content']}</div></div></div>""", unsafe_allow_html=True)

    with st.form("chat_form", clear_on_submit=True):
        q = st.text_area("Pregunta (teoría, mercados, políticas, finanzas corporativas, etc.)",
                         height=120, placeholder="Ej: ¿Impacto de subir la tasa 100 pb en una economía abierta?")
        col1, col2, col3 = st.columns([1,1,2])
        with col1: send = st.form_submit_button("Enviar 💬", use_container_width=True)
        with col2: clear = st.form_submit_button("Limpiar 🧹", use_container_width=True)
        with col3: exptype = st.selectbox("Exportar", ["—", "JSON", "Markdown"])
